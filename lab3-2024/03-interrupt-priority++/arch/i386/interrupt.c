...
static void add_requests(int irq_num)
{
	...
	while (ih) {
		...
		irq->ihndlr = ih;
		irq->started = FALSE;
		irq->prio = irq->ihndlr->prio;
		if (irq->ihndlr->irq_num > SOFT_IRQ) //reduce priority only to simulated interrupts
			irq->prio -= prio_reduction;
...
void arch_interrupt_handler(int irq_num)
{
	...
	if (irq_num < INTERRUPTS && (ih = list_get(&ihandlers[irq_num], FIRST)))
	{
		...
		while (first && first->started == FALSE)
		{
			...
			//set priority reduction only on simulated interrupts completion
			if (first->ihndlr->irq_num > SOFT_IRQ)
				prio_reduction += 3;
